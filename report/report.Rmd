...
```{r TGI, results='asis', echo=FALSE}
    ...
    knit_print(kable(response_list[order(response_list$Arms),], digits=2, row.names = FALSE, booktabs = TRUE) %>%
        kableExtra::kable_styling(full_width = TRUE, position = "left", latex_options = 'scale_down'))
...
```{r individual study ORC plot, results='asis', echo=FALSE, out.width="50%"}
    ...
    for (study in levels(as.factor(plot_data$Study))) {
        ...
        response_list[[study]] <- response_list[[study]]

    }

    output_response <- bind_rows(response_list)

    cat(kable(output_response[order(output_response$Study, output_response$Arms),], digits=2, row.names = FALSE) %>%
...
```{r individual study waterfall, results='asis', echo=FALSE}
...
```{r individual study waterfall option text, results='asis', echo=FALSE}
    cat("Waterfall plots were generated using the metric: ")
...
```{r ANOVA, results='asis', echo=FALSE}
    ...
    for (study in levels(as.factor(plot_data$Study))) {
        ...
        for (tumor in levels(as.factor(df$Tumor))) {
            ...
            if(is.null(aov_res)) {
                cat("ANOVA failed. This is likely due to an invalid study day selection (i.e., study day selected beyond end of available study dates).") 
            } else {
                cat(kable(aov_res$anova[[1]], digits=2) %>%
                ...
                cat(kable(mct, digits=2, booktabs = TRUE) %>%
                kableExtra::kable_styling(full_width = FALSE, position = "right", latex_options = 'scale_down'))
                cat("\n")
                cat("\n")
            }
...
```{r Body Weight Plot, echo=FALSE, eval=weight_plots_available}
