...
# HELPER FUNCTION - INTERPOLATE DATA PROVIDED
  ...
  if (is.null(data) | nrow(data) == 0) {
    ...
    data.tmp <- data.tmp %>%
      ...
      do( data.frame(ID = rep(.$ID, each = .$Day_max+.$adjustment),
                    ...
                    Contributor = rep(.$Contributor, each = .$Day_max+.$adjustment),
                    Study = rep(.$Study, each = .$Day_max+.$adjustment),
    ...
    data.tmp <- data %>%
      dplyr::full_join( data.tmp, by = c("Contributor" = "Contributor", "Study" = "Study", "Tumor" = "Tumor" , "Arms" = "Arms" ,  "ID" = "ID" , "Times" = "Times"))%>%
      arrange(ID, Times)
    ...
    Interpolate.Data <- arrange(Interpolate.Data, ID, Times)%>%
      dplyr::mutate(Arms = na.locf(Arms, na.rm = F),
                    ...
                    Contributor = na.locf(Contributor, na.rm = F),
                    Tumor = na.locf(Tumor, na.rm = F),
...
# HELPER FUNCTION - COMPUTE T/C RATIO
T.C_ratio <- function(data, last.measure.day = NULL) {
  ...
  for (c in levels(as.factor(data$Contributor))) {
...
# HELPER FUNCTION - GET INDIVIDUAL MOUSE RESPONSE LEVEL
# 'dVt', 'log2.Fold.Change', 'AUC.Filtered.Measures', 'AUC.All.Measures', 'ORC'
IndividualMouseResponse <- function(data, last.measure.day = NULL) {
...
# HELPER FUNCTION - CLEAN PLOTLY LEGENDS
  ...
  if (is.null(data) | nrow(data) == 0) {
    ...
    if( level == 'Arm'){
      s.data <- get_data_summary(data, plot.type="normal", measure.var = "Volume",
                            group.vars = c('Contributor', 'Study', "Times", "Arms", "Tumor"))
...
get_plot_scaled <- function(data, orders = NULL, position.dodge, title = NULL, scale.factor, scale.by.volume = FALSE, level = 'Arm', pattern = "TAN", ...){
  ...
  if (is.null(data) | nrow(data) == 0) {
    ...
    if(scale.by.volume) {
      adjusted.data <- data %>%
        arrange(Contributor, Study, Tumor, Arms, ID, Times) %>%
        group_by(Contributor, Study, Tumor, Arms, Times) %>%
        mutate(TimeMean = mean(Volume)) %>%
        ungroup() %>%
        group_by(Contributor, Study, Tumor, Arms, ID) %>%
...
plotAvgGrowthBar <- function(data) {
  ...
  if(length(levels(as.factor(adjusted.data$Tumor))) == 1) {
    p <- p + theme(legend.background = element_rect(fill = 'white', colour = 'black'),
                   legend.title = element_text( size = 12, face = "bold"),
                   legend.text = element_text( size = 12))
  } else {
    p <- p + theme(axis.text.x  = element_text(hjust = 1,vjust = 1,size = 11,angle = 0),
                   legend.background = element_rect(fill = 'white', colour = 'black'),
                   legend.title = element_text( size = 12, face = "bold"),
                   legend.text = element_text( size = 12))# +
      #geom_vline(xintercept = seq(1.5, (length(levels(as.factor(adjusted.data$Tumor))) + 1), by = 1), linetype = "dashed", color = 'gray50')
  }
...
WaterfallPlot_Hybrid <- function(data) {
  ...
  p <- p + theme(legend.background = element_rect(fill = 'white', colour = 'black'),
                 legend.title = element_text( size = 12, face = "bold"),
                 legend.text = element_text( size = 12)) #+
                 #geom_vline(xintercept = seq(0.5, (length(levels(as.factor(data$Tumor))) + 1), by = 1), color = 'gray', linetype = 'dashed')
...
plotTC.ratio <- function(data, plot_measure = c('TC.ratio', 'aov.TC.ratio')) {
  ...
  if(length(levels(as.factor(data$Tumor))) == 1) {
    ...
    p <- p + theme(axis.text.x  = element_text(hjust = 1,vjust = 1,size = 12,angle = 0),
                   legend.background = element_rect(fill = 'white', colour = 'black'),
                   legend.title = element_text( size = 12, face = "bold"),
                   legend.text = element_text( size = 12)) #+
...
plotStackedORC <- function(data) {
  ...
  p <- p + facet_wrap(~ label, dir = 'h', scales = "free_x")
...
# COHORT RECIST VOLUME PLOT
study_volume_plot <- function(data, orders = NULL, position.dodge, title = NULL, ...){
   ...
   if (is.null(data) | nrow(data) == 0) {
    ...
    s.data <- get_data_summary_study(data, measure.var = "Volume", group.vars = c('Contributor', 'Study', "Times", "Arms", "Tumor"))
    s.data$Volume <- round(s.data$Volume, 2)
...
WaterfallPlot_PDX <- function(data,
  ...
  if(length(levels(as.factor(data$Tumor))) > 1) {
    p <- p + facet_grid(. ~ Tumor, scales = 'free_x')
  }
...
EFSplot <- function(data, PercChange_EventSize = 100, plot_on = TRUE) {
  ...
  data <- data %>%
    dplyr::arrange(Tumor, Arms, ID, Times) %>%
  ...
  p <- ggsurvplot(fit,
                  ...
                  legend.labs = label_text,    # Change legend labels
                  xlab = 'Time in Days',
...
response_analysis <- function(input_data, method=c('endpoint.ANOVA','endpoint.KW','mixed.ANOVA','LMM'), last.measure.day = NULL, multi_test_anova = FALSE, tv_study_filtered, tv_tumor_filtered, main_anova_interpolate) {
  ...
  endpoint.data$Tumor=factor(endpoint.data$Tumor)
  ...
  if (is.null(data) | nrow(data) == 0) {
    ...
    if( level == 'Arm'){
      s.data <- get_data_summary(data, plot.type="normal", measure.var = "Body_Weight",
                            group.vars = c('Contributor','Tumor','Study',"Times", "Arms"))
